...
def get_json(walls, goals, ball, name='random'):
    ...
    json += ', "Ball": '
    json += str(ball)
    json += ', "Name": "' + name + '", "Occluders": [], "AbnormWalls": [], "ClosedEnds": [1, 3, 2, 4], "Goals": '
    json += str(goals)
    json += ', "Paddle": null, "BKColor": [255, 255, 255]}'

    return json

def get_trial(name='random', num_rects=10):
        ...
        if not has_overlap:
            ...
            for r in rectangles:
                if check_overlap(screen, ball_rect, r):
                    has_overlap = False
                    break
        #if not has_overlap: # IGNORING THIS BECAUSE OTHER STUFF WILL TAKE CARE OF DISTANCE LATER
    ...
    return get_json(walls, goals, ball, name)
