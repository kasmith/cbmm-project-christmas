...
TrialList = function(triallist, trialpath) {
    ...
    this.mov_pres = triallist.map(x => x[2]);
    this.mov_post = triallist.map(x => x[3]);
    this.goals = triallist.map(x => x[4]);
    this.trials = [];
...
TrialList.prototype.loadTrial = function(trialname, canvas) {
    var idx = this.tnms.indexOf(trialname);
    var tdat = this.trials[idx];
    return [this.loadFromData(tdat, canvas), this.tconds[idx], this.mov_pres[idx], this.mov_post[idx], this.goals[idx]];
};
