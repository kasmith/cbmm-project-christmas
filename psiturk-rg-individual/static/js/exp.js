...
DISPLAY_TIME = 0.5; // Amount of motion to display
RESPONSE_TIME = 2.5; // Max amount of time to produce a response
FLASH_FRAMES = 8; // Amount of time the no motion ball flashes to signal good to go
MAX_TIME = 20; // Max time for a trial to run
...
Experiment.prototype.recordTrial = function(me) {
    ...
    var expcondition = me.pt.taskdata.get('condition');
...
Experiment.prototype.instructions = function() {
    ...
    /* UNCOMMENT TO RUN THE REAL THING
    that2.trial.showinstruct(i1,'black','white', function() {
        that2.trial.showinstruct(i1a, 'black','white', runS1, true);
    }, true );
    */

    // THIS IS FOR SKIPPING INSTRUCTIONS
    that2.trial.showinstruct('starting','black','white',function(){
        that2.trial.score.reset();
        that2.pt.finishInstructions(); // Set participant code to done with instructions
        that2.trial.loadFromTList(that2.loaded.tnms[0], that2.loaded);
        that2.trial.trcounter.display = true;
        that2.run(that2);
    });
