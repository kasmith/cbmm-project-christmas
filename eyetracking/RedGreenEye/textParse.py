...
def quitevent(keys = None, quit_k = [K_LSHIFT,K_ESCAPE]):
    if keys is None:
...
def justifyText(text, font, pos, horizontal=CENTER, vertical=CENTER, color=pg.Color('Black')):
    ...
    if horizontal == 'right':
        ...
        raise Exception('Not valid horizontal justification: ' + str(horizontal))
    ...
    if vertical == 'top':
        ...
        raise Exception('Not valid vertical justification: ' + str(vertical))
...
def displayInstructions(text, screen, keymove=True, clickmove=True, font=FONT_VL, txtcol=pg.Color('Black'),
